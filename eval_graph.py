...
class GraphSemisupervised(object):
    ...
    def setup_train_config(self, batch_size = 128,
                           p_optim = 'Adam', p_lr = 0.0001, p_weight_decay = 0, p_epoch = 2,
...
class Eval_unsuper(object):
    ...
    def __init__(self, dataset, classifier='SVC', log_interval=1, epoch_select='test_max',
                 metric='acc', n_folds=10, device=torch.device('cpu'), **kwargs):
    ...
    def evaluate(self, model, fold_seed=10):
        r"""Run evaluation with given learning model and encoder(s).
        ...
        loader = DataLoader(self.dataset, self.batch_size, shuffle=False)
        t1 = time.time()
        print('----> Get embedding for SVM classification')
        embed, lbls = self.get_embed(model, loader)
        print('----> Extracted {} embeddings for {}-class SVM classification with time={}'.format(embed.shape[0],
                                                                                                  len(np.unique(lbls)), time.time() - t1))
        # lbs = np.array(preprocessing.LabelEncoder().fit_transform(lbls))
        ...
        for fold, (train_index, test_index) in enumerate(kf.split(embed, lbls)):
            ...
            test_score = self.get_clf()(embed[train_index], lbls[train_index],
                                        embed[test_index], lbls[test_index])
            test_scores.append(test_score)
            print('*** fold={}/{} test_score={} with time={}'.format(fold, self.n_folds, test_score, time.time()-t1))
        ...
        # acc = test_scores_m[idx]
        # sd = test_scores_sd[idx]
        print('Best epoch: acc %.4f +/-(%.4f)' % (test_score_mean, test_score_std))
        return test_score_mean, test_score_std

    def evaluate_noprint(self, model, fold_seed=10):
        r"""Run evaluation with given learning model and encoder(s).
    ...
    def evaluate_run(self,run_times,model,fold_seed=10):
        test_score_means=[]

        for i in range(0,run_times):
            score_mean,score_std=self.evaluate(model,fold_seed)
            test_score_means.append(score_mean)

        test_score_means=torch.tensor(test_score_means)


        mean=test_score_means.mean().item()
        std2=test_score_means.std().item()

        print('Final result for ', run_times, ' runs: acc %.4f +/-(%.4f)' % (mean, std2))

        return mean,std2

    def grid_search(self, learning_model, fold_seed=12345,
                    p_lr_lst=[0.1, 0.01, 0.001], p_epoch_lst=[20, 40, 60]):
        r"""Perform grid search on learning rate and epochs in pretraining.
        ...
        for p_lr in p_lr_lst:
            for p_epoch in p_epoch_lst:
                ...
                #enc = copy.deepcopy(encoder)
                #acc_m, acc_sd = self.evaluate(model, enc, fold_seed)
                acc_m, acc_sd = self.evaluate_noprint(model, fold_seed)
                acc_m_lst.append(acc_m)
                acc_sd_lst.append(acc_sd)
                paras.append((p_lr, p_epoch))
        idx = np.argmax(acc_m_lst)
        ...
        return acc_m_lst[idx], acc_sd_lst[idx]

    def gridsearch_run(self,run_times,model,fold_seed=10):
        test_score_means = []

        for i in range(0,run_times):
            score_mean, score_std = self.grid_search(model, fold_seed)
            test_score_means.append(score_mean)

        test_score_means = torch.tensor(test_score_means)
        mean = test_score_means.mean().item()
        std2 = test_score_means.std().item()
        print('Final result for 10 runs grid search: acc %.4f +/-(%.4f)' % (mean, std2))
...
class Eval_semi(object):
    ...
    def __init__(self, dataset, label_rate=1, loss=nn.functional.nll_loss,
                 epoch_select='test_max', metric='acc', n_folds=10, device=torch.device('cpu'), **kwargs):
    ...
    def evaluate(self, pre_model, pred_head=None, fold_seed=12345):
        r"""Run evaluation with given learning model and encoder(s).
        ...
        :rtype: (float, float)
        """
        model = PredictionModel(pre_model.encoder, pred_head, pre_model.d_out, self.out_dim).to(self.device)
    ...
    def evaluate_run(self,run_times,model):
        test_scores=[]
        for i in range(0,run_times):
            test_score,_=self.evaluate(model)
            test_scores.append(test_score)

        test_scores=torch.tensor(test_scores)
        mean=test_scores.mean().item()
        std=test_scores.std().item()

        print('Final result for ', run_times,' runs: acc %.4f +/-(%.4f)' % (mean, std))

        return mean,std

    def gridsearch_run(self,run_times,model):
        test_scores=[]
        for i in range(0,run_times):
            test_score,_,_=self.grid_search(model)
            test_scores.append(test_score)
        ...
        print('Final result for grid search', run_times,' runs: acc %.4f +/-(%.4f)' % (mean, std))

        return mean,std

    def grid_search(self, learning_model, pred_head=None, fold_seed=12345,
                    p_lr_lst=[0.1, 0.01, 0.001, 0.0001], p_epoch_lst=[20, 40, 60, 80, 100]):
        ...
        for p_lr in p_lr_lst:
            for p_epoch in p_epoch_lst:
                self.setup_train_config(f_lr=p_lr, f_epoch=p_epoch)
                acc_m, acc_sd = self.evaluate(learning_model, pred_head, fold_seed)
                acc_m_lst.append(acc_m)
                acc_sd_lst.append(acc_sd)
                paras.append((p_lr, p_epoch))
        idx = np.argmax(acc_m_lst)
